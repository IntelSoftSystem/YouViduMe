...
class RecentList extends Component<Props, *> {
    ...
    _onNavigateToConference(conference: RecentListItem) {
        const currentConferenceDate = moment.utc(conference.startTime).format("YYYY-MM-DD HH:mm");
        fetch(
            `https://vmpuapi.azurewebsites.net/api/meeting/CheckMeetingDate/${currentConferenceDate}/${conference.room}/`
        )
        .then(res => res.json())
        .then(
         (result) => {
            ...
            Object.keys(dict).map(function(key){dictKey = key;}) ;
